...
if [[ "$unamestr" == 'Linux' ]]; then
	...
	avr-gcc -Os -std=c11 -ffunction-sections -fdata-sections -mmcu=atmega328p -DF_CPU=16000000UL -Wl,--gc-sections -I${path_to_om} -I${path_to_mdd} MDD_ldr_led_main.c -o MDD_ldr_led   
	if [ $? -ne 0 ]; then
		exit 1
	fi
	avr-objcopy -O ihex -R .eeprom MDD_ldr_led MDD_ldr_led.hex
	if [ $? -ne 0 ]; then
		exit 1
	fi 
	avrdude -F -V -c arduino -p ATMEGA328P -P $port -b $b_rate -U flash:w:MDD_ldr_led.hex
	if [ $? -ne 0 ]; then
		exit 1
	fi
fi
